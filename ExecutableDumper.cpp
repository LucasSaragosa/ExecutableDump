...
std::string repl(std::string str, const std::string& from, const std::string& to) {
    size_t start_pos = 0;
...
extern const uint64_t crc_tab[];

uint64_t crc(const char* str) {
    uint64_t crc = 0;
    int len = strlen(str);
    for (int i = 0; i < len; i++) {
...
//from dbghelp.dll
typedef DWORD (__stdcall *UndecorFunc)(PCSTR DecoratedName, PSTR UnDecoratedName, DWORD UndecoratedLength, DWORD Flags);

#define GETBOOL(var,msg) std::cout << msg << "?" << std::endl; while(true){ c = std::cin.get(); if(c == 'y' || c == 'n') {var = c == 'y'; break;}}

//undecorate a lot of the time doesn't work, so just in case this should allow a few more to be deciphered
std::string try_again(const std::string& in);

int main(int argn,char** argv)
{
    std::cout << "\n\n- Executable Dumper v1.1.2 - By Lucas Saragosa\n- You can use the following options (all are optional), or enter them as the program runs\n\n\t\t\t\t------ Options ------\
        ...
        \n\t-i <input file> : Specify the input file (max 200 characters)\n\t-o <output file> : Specify the output file (max 200 characters)\n\
    ...
    int min = 3;
    bool an = false;
    bool crc = false, rttiinfo = false, verbose = false;
    bool hasargs = argn > 1;
    char* f;
    for (int i = 1; i < argn; i++) {
        ...
        switch (argv[i][1]) {
            ...
            case 'o':
                if (i + 1>= argn) {
                    std::cout << "At argument " << i << ", expected an output file" << std::endl;
                    return 4;
                }
                f = argv[1+i];
                if (strlen(f) > 200) {
                    std::cout << "File name to long for the output file" << std::endl;
                    return 5;
                }
                strcpy(outfile, f);
                i++;
            ...
            case 'i':
                if (i + 1 >= argn) {
                    std::cout << "At argument " << i << ", expected an input file" << std::endl;
                    return 4;
                }
                f = argv[1+i];
                if (strlen(f) > 200) {
                    std::cout << "File name to long for the input file" << std::endl;
                    return 5;
                }
                i++;
                strcpy(infile, f);
                break;
            default:
                std::cout << "Warning: Ignoring argument " << argv[i] << ", did not match a valid one" << std::endl;
                break;
        }
    }
    int c;
    static const char* clazz = "AV";
    static const char* struc = "AU";
    static const char* clazzns = "::AV";
    static const char* strucns = "::AU";
    if (!infile[0]) {
        std::cout << "Enter the input file" << std::endl;
        std::cin.getline(infile, 200);
    }
    std::ifstream stream(infile, std::ios::binary);
    if (stream.fail()) {
        std::cout << "Couldn't open that file to read :(";
        return 1;
    }
    int tot = 0;
    if (!outfile[0]) {
        std::cout << "Enter the output file" << std::endl;
        std::cin.getline(outfile, 200);
    }
    std::ofstream ostream(outfile);
    if (ostream.fail()) {
        std::cout << "Couldn't open that file to write :(";
        return 1;
    }
    stream.seekg(0, 2);
    uint64_t z = stream.tellg();
    stream.seekg(0, 0);
    std::string curr;
    uint64_t off = -1;
    uint64_t last = 0;
    HMODULE dbg = NULL;
    UndecorFunc func = NULL;
    if (!hasargs) {
        GETBOOL(rttiinfo, "Dump only the classes and structs? (y/n)")
        GETBOOL(crc, "Dump the CRC after (useful for telltale games)? (y/n)")
    }
    if (rttiinfo) {
        if (dbg = LoadLibrary(L"dbghelp.dll")) {
            func = (UndecorFunc)GetProcAddress(dbg, "UnDecorateSymbolName");
        }
    }
    int per = 0;
    for (;;) {
        ...
        if (!token) {
            if (curr.length() >= min) {
                ...
                if (rttiinfo) {
                    ...
                    char* out = new char[0x1000];
                    if (!out) {
                        std::cout << "could not allocate memory :(" << std::endl;
                        return 2;
                    }
                    if(func)func(curr.c_str() + 1, out, 0x1000, 0);
                    std::string str(out);
                    str = repl(str, clazzns, "");
                    str = repl(str, strucns, "");
                    if (str.find("@") == str.npos) {
                        str = str.substr(4);
                        if (!str.find(clazz)) {
                            str = str.substr(2);
                        }else  if (!str.find(struc)) {
                            str = str.substr(2);
                        }
                    }
                    else {
                        if (verbose)std::cout << "Undecorating RTTI class/struct name without debug helper: " << curr.c_str() << std::endl;
                        str = try_again(str.substr(4));
                    }
                    DOCRC(str)
    ...
    stream.close();
    ostream.close();
    std::cout << "Dumped all strings from " << infile << " to " << outfile << ".\nFound a total of " << tot << " valid strings that matched the input parameters.";
}

std::string try_again(const std::string& in) {
    std::string ret = in.substr(1);//remove the $
    bool mapfix = false;
    size_t pos;
    std::string val("");
    if ((pos=ret.find("@HV")) != ret.npos) {//most likely a map, just going to dump it as one bc why not
        mapfix = true;
        pos += 2;
        while (true) {
            pos++;
    ...
    ret = repl(ret, "@PE", "<");
    ret = repl(ret, "@@U?$", ",less");
    ret = repl(ret, "lessless", "less");
    ret = repl(ret, "@std@", "");
    ret = repl(ret, "<AV", "<");//class x
    ret = repl(ret, "<AU", "<");//struct x
    ret = repl(ret, "?", "");
    ret = repl(ret, "$", "");
    for (int i = 0; i < ret.length(); i++) {
        int ch = ret[i];
        if (i >= 1 && ch == '@' && i + 1 < ret.length() && isalpha(ret[i + 1]) && isalpha(ret[i - 1])) {
            std::string sub("");
            std::string head("");
            for (int x = i + 1; x < ret.length(); x++) {
                if (!isalpha(ret[x]))break;
                head += ret[x];
            }
            for (int x = i - 1; x > 0; x--) {
                if (!isalpha(ret[x]))break;
            ...
            ret = repl(ret, sub + "@" + head, head + "::" + sub);
            break;
        }
;    }
    ret = repl(ret, "@", "");
    int x = 0;
    if ((!ret.find("map") || !ret.find("Map") || !ret.find("set") || !ret.find("Set")) && ((x=ret.find("<")) != ret.npos)) {
        int first = ret.find("<", x);
        if (first != ret.npos) {
            int second = ret.find("<", first);
            if (second != ret.npos) {
                ret[second] = ',';
            }
            ret[first] = ',';
    ...
    int rem = std::count(ret.begin(), ret.end(), '<');
    for (int i = 0; i < rem; i++) {
        ret += '>';
    }
    return ret;
